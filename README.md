...
此節僅供自行建置原始碼：Node.js 20.19+（20.x）或 22.12+、Python 3.10+（純標準庫，無 pip 依賴）。Windows 免安裝 ZIP 不需要先完成這些開發環境步驟。
...
其他相容服務須提供 `chat/completions` 文字回覆 API。能列出模型時直接選取；只有明確缺少模型清單端點的服務才允許手填，之後仍須測試真正回覆。此版本不支援工具呼叫、圖片或語音輸入。
