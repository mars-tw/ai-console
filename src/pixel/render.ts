...
export class OfficeRenderer {
  ...
  private drawOneAgent(b: CanvasRenderingContext2D, world: World, a: Agent, o: DrawOpts) {
    ...
    //
    // 這裡連續踩過三次，記錄下來免得又改回去：
    //   1. 最早疊一塊 saturation 混色方塊 → 混色作用於「方塊底下的所有東西」，
    //      地板家具一起被抽掉彩度，畫面上多出一個硬邊灰方塊
    //   2. 改成 filter: grayscale → 方塊沒了，但整隻變灰。黃色的傻龍看起來
    //      像壞掉的素材
    //   3. 改成半透明 → 還是像沒載好的圖，而且「查不到狀態」跟「東西壞了」
    //      在畫面上長得一樣
    //
    // 根本問題是拿視覺效果去表達狀態。這個場景的其他狀態全部是用**行為與位置**
    // 表達的：工作坐工位、休息去沙發、派工走到白板、閒晃四處走動。
    // 只有這一個額外去動精靈本身，那才是不一致的地方。
    // 沒有活動紀錄的角色改成「站著不動」來表達，名牌與圖例負責說明原因。
