...
try {
  ...
  execFileSync('python', [path.join(root, 'scripts/audit_release.py'), stage, '--kind', 'stage', '--version', manifest.version, '--check-local-pairing'], {
  ...
  for (const directory of paths) execFileSync('python', [path.join(root, 'scripts/audit_release.py'), directory, '--kind', 'windows', '--version', manifest.version, '--check-local-pairing'], {
    cwd: root, stdio: 'inherit', windowsHide: true,
  })
